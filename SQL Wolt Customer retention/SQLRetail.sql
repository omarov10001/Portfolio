...
-- Separate the users depending of the product line (Retail store)
with firstretail as (select * from first_purchases$ where product_line='Retail store')
,retail as (select * from purchases$ where product_line='Restaurant')
...
from retail A left join cohort_items as c on A.user_id=c.user_id 
